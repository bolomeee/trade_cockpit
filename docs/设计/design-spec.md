...
> 最后更新：2026-07-11（TopNav 主题切换控件归入操作按钮组）
...
```
┌─ TopNav（64px，保留）──────────────────────────┐
│  MA150 Tracker · Last refresh · [主题 | 刷新 | …] │
...
- 右（当前实现）：
  ...
  - 所有图标/操作控件置于同一个水平 `ButtonGroup`；主题切换为该组第一个控件，位于 Last refresh 右侧。不得将主题切换作为独立按钮放在该按钮组外；其后的刷新、AI 摘要（如适用）、布局与设置操作按路由条件出现。
    - 深色模式显示 `Sun` 图标，`aria-label="切换到浅色模式"`，Tooltip 为“浅色模式”；浅色模式对应显示 `Moon`、“切换到深色模式”与“深色模式”。
    - 主题切换影响全局 light/dark 主题，不因路由而隐藏。
...
```
┌─ TopNav（共享，64px）──────────────────────────────────────────────┐
│  MA150 Tracker · [Workbench] [Cockpit] [News] · Last refresh · [主题 | 刷新 | ResetLayout | Settings | …] │
...
- TopNav 右侧的主题切换是操作 `ButtonGroup` 的首项，紧接 Last refresh；后续的刷新、布局与设置操作与其保持同组、连续边框。
...
触发入口：Cockpit 页面 TopNav 右侧操作 `ButtonGroup` 中的“齿轮 (Settings)”按钮（仅 `/cockpit` 路由可见；与主题、刷新及 Reset Layout 保持同组）。
