...
last_modified_by: system-design (F222 2026-07-02 — 新增 PUT /api/watchlist/{ticker}/color；GET /api/signals 追加 labelColor)
---
...
### PUT /api/watchlist/{ticker}/color
> Feature：F222 Watchlist 颜色标记

**用途**：设置或清除指定 ticker 的颜色标记
**认证**：不需要
...
{ "color": "red" }
```
`color`：枚举 `"red" | "yellow" | "blue" | null`；传 `null` 清除标记，恢复无色

**成功响应（200）**：
```json
{
  "data": { "ticker": "AAPL", "labelColor": "red" },
  "message": "success"
}
```

**说明**：
- 单选态：新值直接覆盖旧值，不支持多色叠加
- 与 `dataStatus` 无关，`loading` / `insufficient` 状态下也可设置颜色
...
|------|--------|------|
| ticker 不在 watchlist 中 | NOT_FOUND | 404 |
| `color` 不是 red/yellow/blue/null 之一 | VALIDATION_ERROR | 422 |
...
### GET /api/signals
> Feature：F002 信号引擎, F004 SignalBoard, F222 Watchlist 颜色标记

**用途**：获取所有 watchlist 股票的最新信号（SignalBoard / Watchlist 表格数据源）
**认证**：不需要
...
- 每只股票只返回最新一天的信号
- `labelColor`（F222）：枚举 `"red" | "yellow" | "blue" | null`，来自 `Stock.label_color`；由 `PUT /api/watchlist/{ticker}/color` 设置
...
## Refresh Health（/api/refresh-health）

### GET /api/refresh-health
> Feature：F221 刷新可见化（定时任务健康度）

**用途**：聚合定时刷新任务的数据新鲜度与近期错误数，供 TopNav 告警徽标判断是否有刷新任务陈旧/失败。**只读，不触发任何刷新。**
**认证**：不需要
...
{
  "data": {
    "universe": { "lastAt": "2026-06-25T07:08:23Z", "ageDays": 0.3, "stale": false },
    "breakout": { "lastAt": "2026-06-25T07:17:58Z", "ageDays": 0.3, "stale": false },
    "poolCacheRows": 224,
    "recentErrors": 0
  },
  "message": "success"
...
| 字段 | 类型 | 说明 |
|------|------|------|
| universe.lastAt | string\|null | `market_scan_universe` 最新 `last_seen_at`（无数据为 null） |
| universe.ageDays | number\|null | 距今天数（无数据为 null） |
| universe.stale | boolean | `ageDays > 8` 或无数据 → true（universe 每周刷新） |
| breakout.lastAt | string\|null | `market_breakout_scans` 最新 `scanned_at` |
| breakout.ageDays | number\|null | 距今天数 |
| breakout.stale | boolean | `ageDays > 3` 或无数据 → true（scanner 每工作日，含周末缓冲） |
| poolCacheRows | integer | `cockpit_pool_cache` 当前行数 |
| recentErrors | integer | 近 24h 内 ERROR 级 `system_logs` 条数 |
